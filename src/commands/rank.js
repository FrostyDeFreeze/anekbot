module.exports = {
	...
	async execute(client, ctx, utils) {
		...
		const getRankInfo = rank => `${rank} [${ranks[rank].name}]`
		...
		switch (ctx.args[0]?.toLowerCase()) {
			...
			default: {
				return {
					text: `Твой текущий ранг: ${getRankInfo(currRank)} \u{2027} Следующий ранг: ${getRankInfo(
						nextRank
					)} \u{2027} Необходимо: ${nextRankCost} \u{2027} У тебя: ${balance.toFixed(1)} \u{2027} ${
