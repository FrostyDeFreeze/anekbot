...
module.exports = {
	...
	async execute(client, ctx, utils) {
		...
		try {
			...
			if (data.length === 0) {
				return {
					text: `У пользователя нет недавних треков`,
					reply: true
				}
			}
			...
			const status = isNowPlaying
				? `Сейчас играет \u{25B6}`
				: `(${bb.utils.humanizer(decreaseHours(new Date(), 3) - new Date(data[0].date[`#text`]), {
						largest: 2
				  })} назад) Последний сыгранный \u{23EF}`
