module.exports = {
	...
	async execute(client, ctx, utils) {
		...
		let text = `Поздравляю! Ты отгадал(а) слово "${data[ctx.channel.id].word}" и получаешь 25 монет \u{2027} Благодаря тебе ${
			data[ctx.channel.id].describer.login
		} также получает 25 монет за красноречивое объяснение \u{1F913}`

		if (data[ctx.channel.id].isFailedToWhisper) {
			describerReward = false
			text = `Поздравляю! Ты отгадал(а) слово "${data[ctx.channel.id].word}" и получаешь 25 монет \u{1F913}`
		}
		...
		if (guess.toLowerCase() === data[ctx.channel.id].word.toLowerCase()) {
			...
			describerReward ? bb.utils.coins.addCoins(data[ctx.channel.id].describer.id, ctx.channel.id, 25) : null
